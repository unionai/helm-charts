...
User-facing tooling to **stand up a Union dataplane on a cluster and smoke-test
...
| `.github/ci-scripts/ci_dataplane.py` *(planned move → `ops.py`)* | control-plane ops: `setup-routing` \| `run-smoke-suite` \| `wait-healthy` \| `teardown` |
...
```bash
tools/dataplane/k3d/up.sh --from-aws-secret selfmanaged/canary/helm-charts-ci/sm-k3d-dp-1/operator --smoke
