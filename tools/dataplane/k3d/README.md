...
CI k3d leg (integration-checks.yaml) — a single-node cluster with an embedded registry,
...
# also run the functional tests after install
tools/dataplane/k3d/up.sh --from-aws-secret <name> --functional
...
6. Waits for the cluster to register healthy on the control plane
   (`integration_ops.py wait-healthy`), and optionally runs the functional tests.
