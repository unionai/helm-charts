...
- **RBAC scope now follows the deployment's namespace scope, not the raw `low_privilege` flag** (Linear UN-29; RBAC hardening phase 0). Six RBAC templates branched on `.Values.low_privilege` in 18 copy-pasted places (role kind, binding kind, `roleRef.kind`), while the rest of the chart keys single-namespace behavior off the `singleNamespace` helper — `or (not namespaces.enabled) low_privilege`. The two disagreed for exactly one combination, **`namespaces.enabled: false` + `low_privilege: false`**, which puts the chart in single-namespace mode (all task pods land in the release namespace, `clusterresourcesync` is suppressed) while still emitting cluster-scoped RBAC for the operator, executor, leaseworker, webhook, proxy and propeller. All 18 sites now resolve through the new `dataplane.rbacKind` / `dataplane.rbacBindingKind` helpers.

  **In that combination only**, five roles move from `ClusterRole` + `ClusterRoleBinding` to `Role` + `RoleBinding` in the release namespace: **`flytepropeller-role`, `union-executor`, `union-leaseworker`, `proxy-system` and `union-webhook-role`**. Rule content is carried over byte-for-byte — only the object kind and the binding kind change. `operator-system` is deliberately untouched and stays cluster-scoped; it has a different template shape and is handled in a later hardening phase.

  **Genuine multi-namespace deployments (`namespaces.enabled: true`) are unaffected** and keep cluster-scoped RBAC, as are all `low_privilege: true` deployments, which were already namespaced.
...
- **BREAKING for `namespaces.enabled: false` + `low_privilege: false` — confirm nothing depends on the cluster-scoped RBAC first.** The five roles above (`flytepropeller-role`, `union-executor`, `union-leaseworker`, `proxy-system`, `union-webhook-role`) lose cluster scope in that mode. This is the correct scope — every task pod already lands in the release namespace there — but if anything outside this chart binds to those `ClusterRole`s by name, or if a component in your environment writes to a namespace other than the release namespace, it will start getting `Forbidden`. Verify against a non-production data plane before rolling out. No other mode changes: `namespaces.enabled: true` and `low_privilege: true` deployments render identically to the previous release.
- **ArgoCD with pruning disabled: the old cluster-wide grant survives the upgrade.** The chart stops *creating* the `ClusterRole` and `ClusterRoleBinding`; it cannot delete objects it no longer renders. A plain `helm upgrade` removes resources that disappear from the manifest, but ArgoCD only does so when pruning is enabled for the Application. With pruning off, the pre-existing `ClusterRole` and `ClusterRoleBinding` stay in the cluster, the ServiceAccount keeps its cluster-wide grant through the surviving binding, and **the security improvement silently does not take effect** — the sync reports healthy and the render looks correct while the cluster is unchanged. Either enable pruning for the data-plane Application, or after upgrading delete the leftover `ClusterRoleBinding`s (and their `ClusterRole`s) for the five names above by hand.
