...
#
# `kubectl get` failure is only treated as success when stderr matches
# NotFound. Other failures (network, RBAC, TLS, apiserver timeout) are
# propagated so the Job retries via backoffLimit instead of declaring
# success while the resource is still present.
delete_wait() {
  local kind=$1 name=$2
  local err
  kubectl delete "$@" --cascade=foreground --ignore-not-found --wait=false
  for _ in $(seq 1 360); do
    if err=$(kubectl get "$@" 2>&1 >/dev/null); then
      sleep 2
      continue
    fi
    case "$err" in
      *NotFound*|*"not found"*) return 0 ;;
      *) echo "kubectl get $* failed: $err" >&2; return 1 ;;
    esac
  done
...
#
# As in delete_wait, NotFound is the only `kubectl get` failure treated as
# "absent". Other failures propagate so the Job retries instead of
# silently skipping adoption (which would resurface as an `invalid
# ownership metadata` failure on the subsequent dataplane upgrade).
adopt_if_exists() {
  local crd=$1
  local err
