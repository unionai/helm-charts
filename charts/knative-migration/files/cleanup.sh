...
#    this design avoids.
#
#    Idempotency guard: on rerun after a successful prior cleanup the
#    operator CRD has been deleted by step 3, so `kubectl delete
#    knativeserving` fails with "the server doesn't have a resource type"
#    before `--ignore-not-found` applies. If the CRD is absent, there is
#    by construction no CR left to clean up; skip the delete entirely.
